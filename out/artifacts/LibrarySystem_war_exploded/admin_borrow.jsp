...
<script>
    ...
    function load(currentPage) {
        ...
        $.post("adminSelectBorrowServlet",{currentPage:currentPage},function (pb) {
            ...
            for(var i=0;i<pb.list.length;i++){
                var data=pb.list[i];
                var date=new Date();
                if(data.returndate>date.getTime()) {
                    var li = '<tbody>\n' +
                        '                            <td>' + data.barcode + '</td>\n' +
                        '                            <td>' + data.username + '</td>\n' +
                        '                            <td>' + data.type + '</td>\n' +
                        ...
                        '                            <td><button type="button" class="btn btn-info btn-xs" data-toggle="modal" onclick="borrowbook(\'' + data.barcode + '\',\'' + data.username + '\',\''+data.returndate+'\')">还书</button></td>\n' +
                        '</tbody>';
                }else{
                    var li = '<tbody>\n' +
                        '                            <td>' + data.barcode + '</td>\n' +
                        ...
                        '                            <td>' + timestampToTime(data.borrowdate) + '</td>\n' +
                        '                            <td>' + timestampToTime(data.returndate) + '</td>\n' +
                        '                            <td><button type="button" class="btn btn-danger btn-xs" data-toggle="modal" onclick="borrowbook(\'' + data.barcode + '\',\'' + data.username + '\',\''+data.returndate+'\')">还书</button></td>\n' +
                        '</tbody>';
    ...
    //借阅按钮函数
    function borrowbook(barcode,username,returndate) {
        var date=new Date();
        if(returndate>date.getTime()) {
            con = confirm("是否还书?");
            if (con == true) {
                location.href = "adminReturnServlet?barcode=" + barcode + "&username=" + username;
            }
        }else{
            var time=date.getTime()-returndate;
            con = confirm("是否还书?");
            if (con == true) {
                location.href = "alipayServlet?barcode=" + barcode + "&username=" + username+"&time="+time;
            }
        }
    };
</script>
