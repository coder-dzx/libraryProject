...
<script>
    ...
    function load(currentPage) {
        $.post("selectBorrowServlet",{currentPage:currentPage},function (pb) {
            ...
            for(var i=0;i<pb.list.length;i++){
                ...
                if(data.returndate>date.getTime()){
                    var li='<tbody>\n' +
                        ...
                        '                            <td><button type="button" class="btn btn-info btn-xs" data-toggle="modal" onclick="borrowbook(\''+data.barcode+'\',\''+data.returndate+'\')">还书</button></td>\n' +
                        '</tbody>';
                }else{
                    var li='<tbody>\n' +
                        '                            <td>'+data.barcode+'</td>\n' +
                        ...
                        '                            <td>'+timestampToTime(data.borrowdate)+'</td>\n' +
                        '                            <td>'+timestampToTime(data.returndate)+'</td>\n' +
                        '                            <td><button type="button" class="btn btn-danger btn-xs" data-toggle="modal" onclick="borrowbook(\''+data.barcode+'\',\''+data.returndate+'\')">还书</button></td>\n' +
                        '</tbody>';
    ...
    //借阅按钮函数
    function borrowbook(barcode,returndate) {
        var date=new Date();
        if(returndate>date.getTime()){
            con=confirm("是否还书?");
            if(con==true){
                location.href = "returnServlet?barcode="+barcode;
            }
        }else{
            var time=date.getTime()-returndate;
            con=confirm("是否还书?");
            if(con==true){
                location.href = "alipayServlet?barcode="+barcode+"&time="+time;
            }
        }
    };
