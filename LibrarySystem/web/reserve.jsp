...
<script>
    ...
    function load(name,currentPage) {
        ...
        $.post("selectReserveServlet",{book_name:name,currentPage:currentPage},function (pb) {
            ...
            for(var i=0;i<pb.list.length;i++){
                var data=pb.list[i];
                    var li='<tbody>\n' +
                        ...
                        '                            <td>'+data.author+'</td>\n' +
                        '                            <td>'+data.place+'</td>\n' +
                        '                            <td><button type="button" class="btn btn-danger btn-xs" data-toggle="modal" onclick="borrowbook(\''+data.barcode+'\')">取消预约</button></td>\n' +
                        '</tbody>';
                list+=li;
            }
            $("#data_list").html(list);
    ...
    function borrowbook(barcode) {
            con=confirm("是否取消预约?");
            if(con==true){
                location.href = "updateReserveServlet?barcode="+barcode;
            }
    }
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default bootstrap-admin-no-table-panel">
                        <div class="panel-heading">
                            <div class="text-muted bootstrap-admin-box-title">预约查询</div>
                        </div>
