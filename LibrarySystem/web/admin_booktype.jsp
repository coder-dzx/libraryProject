...
<script>
    ...
    $(function () {
        load(1);
        //修改表单的提交事件
        $("#updateType_submit").submit(function () {
            $.post("adminUpdateTypeServlet",$(this).serialize(),function (data) {
        ...
        //添加表单的提交事件
        $("#addType_submit").submit(function () {
            $.post("adminAddTypeServlet",$(this).serialize(),function (data) {
    ...
    function load(currentPage) {
        ...
        $.post("adminSelectTypeServlet",{currentPage:currentPage},function (pb) {
            ...
            for(var i=0;i<pb.list.length;i++){
                ...
                var li='<tbody>\n' +
                    '                            <td>'+data.typename+'</td>\n' +
                    '                            <td><button type="button" class="btn btn-warning btn-xs" data-toggle="modal" data-target="#updateModal" id="btn_update" onclick="showtypeinfo(\''+data.typename+'\',\''+data.id+'\')">修改</button>\n' +
                    '                            <button type="button" class="btn btn-danger btn-xs" onclick="deleteTypeName(\''+data.typename+'\')">删除</button></td>\n' +
                    '</tbody>';
                list+=li;
            }
            $("#data_list").html(list);
    ...
    //修改按钮函数
    function showtypeinfo(typename,id) {
        $("#updateTypeName").val(typename);
        $("#updateTypeId").val(id);
    }
    //删除按钮函数
    function deleteTypeName(typename) {
        con = confirm("是否删除?");
        if(con == true) {
            location.href = "adminDeleteTypeServlet?typename=" + typename;
        }
    }
            ...
            <div class="row">
                <div class="col-lg-12">
                    <table id="data_list" class="table table-hover table-bordered" cellspacing="0" width="100%">
                        ...
                        <td><button type="button" class="btn btn-warning btn-xs" data-toggle="modal" data-target="#updateModal" id="btn_update" onclick="showInfo2()">修改</button>
                            <button type="button" class="btn btn-danger btn-xs" onclick="deletebook()">删除</button></td>
                        </tbody>
                    </table>
                </div>
            </div>
